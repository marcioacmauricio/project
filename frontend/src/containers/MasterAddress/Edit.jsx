...
import { createMasterAddress, readMasterAddress, updateMasterAddress } from 'reducers/MasterAddress/Actions'
import MasterAddressModel from 'models/MasterAddressModel'
import Fields from 'fields/Fields'
import { MenuHeader, HeaderAdmin } from 'components/Headers'
class MasterAddressEdit extends React.Component {
	constructor() {
		super()
	...
	onSubmit(e) {
		...
		if (this.props.match.params.id > 0){
			this.props.updateMasterAddress(Values);
		} else {
			this.props.createMasterAddress(Values);
		}
		e.preventDefault();	   
	}
	componentWillMount() {
	...
	componentWillReceiveProps(nextProps) {
		if (( this.props.match.params.EntityPredesc !== "" ) && ( this.props.match.params.EntityPredesc !== undefined ) && ( this.props.match.params.EntityPredesc !== null ) && (nextProps.Payload.Item !== null)){
			if (nextProps.Payload.Item.id > 0){
				this.props.history.push(`/Admin/MasterAddress/EditItem/${nextProps.Payload.Item.id}`)
				return
			}			
		}
						...
						<Card className="shadow">
							<CardHeader className="border-0">
								...
								<ButtonGroup>
							        ...
							        <Link className="btn btn-outline-info" to={`/Admin/MasterAddress/ListItems/MasterUser/${ItemValue}`}><i className="fa fa-times" aria-hidden="true"></i> Fechar</Link>
								</ButtonGroup>
							</CardHeader>
							<CardBody>
								...
								<CardText tag="div">
									{this.renderAlert()}
									<Form>
										{this.renderField('postal_code')}
										{this.renderField('address_residence')}
										{this.renderField('city')}
										{this.renderField('country')}
										{this.renderField('estate')}
										{this.renderField('neighborhood')}
									</Form>
								</CardText>
							</CardBody>
							<CardFooter>
...
export default connect(mapStateToProps, {readMasterAddress, createMasterAddress, updateMasterAddress})(MasterAddressEdit);