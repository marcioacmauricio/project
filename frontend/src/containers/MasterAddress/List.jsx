...
import { fetchMasterAddress, deleteMasterAddress } from '../../reducers/MasterAddress/Actions'
import { connect } from 'react-redux'
import MasterAddressModel from 'models/MasterAddressModel'
...
class MasterAddressList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
		...
		this.ColumnsList = ['id', 'first_name', 'last_name', 'id_user']
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let MasterAddress = {}
		if (AllFilters.MasterAddress !== undefined){
			MasterAddress = AllFilters.MasterAddress
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (MasterAddress[ColumnName] !== undefined){
				Param = MasterAddress[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.MasterAddress = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = MasterAddressModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchMasterAddress(Post, this.props.match.params.IdPredesc);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = MasterAddressModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/MasterAddress/NewItem/MasterUser/${this.props.match.params.IdPredesc}`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/MasterAddress/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/MasterAddress/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
					...
					<div className="col">
						<Card className="shadow">
							<CardHeader className="border-0">
								<MenuHeader Entity="MasterAddress" ItemValue={ItemValue} ItemLabel={ItemLabel} View="List" />
							</CardHeader>		
							<CardBody>
								<Row>
									<Col xs="3">
...
const mapStateToProps = (state, props) => {
	return {
		Payload: state.MasterAddressReducer.item,
		Payloads: state.MasterAddressReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchMasterAddress, deleteMasterAddress})(MasterAddressList);