...
import { readMasterAddress } from 'reducers/MasterAddress/Actions'
...
class MasterAddressView extends React.Component {
	constructor() {
		super();
		this.ColumnsList = ['id', 'ordering', 'state', 'checked_out', 'checked_out_time', 'created_by', 'created_time', 'modified_by', 'modified_time', 'postal_code', 'id_user', 'address_residence', 'city', 'country', 'estate', 'neighborhood']
		this.ColumnsFields = {}
		let ColumnData = {}
		let Item = {}
		...
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			ColumnData = MasterAddressModel.columns[ColumnName]
	...
	componentWillMount() {
		if (this.props.match.params.id > 0){
			...
			this.props.readMasterAddress(Posts);
		}		
	}
	...
	renderColumns(){
		...
		for (let ColumnName in this.state.Item){
			...
			let ColumnData = MasterAddressModel.columns[ColumnName]
					...
					<div className="col">
						<Card className="shadow">
							<CardHeader className="border-0">
								<MenuHeader getItem={this.getItem.bind(this)} Entity="MasterAddress" ItemValue={ this.state.Item.id } ItemLabel={ this.state.Item.first_name } View="View" />	    				
							</CardHeader>
							<CardBody>
							...
							<CardFooter>
								<Button outline color="success" className="float-right" ><Link to={`/Admin/MasterAddress/ListItems/MasterUser/${ItemValue}`}>Lista</Link></Button>							    
							</CardFooter>
						</Card>
					</div>
				</Container>
...
export default connect(mapStateToProps, {readMasterAddress})(MasterAddressView);