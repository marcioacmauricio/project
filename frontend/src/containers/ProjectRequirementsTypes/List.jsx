...
import { fetchProjectRequirementsTypes, deleteProjectRequirementsTypes } from '../../reducers/ProjectRequirementsTypes/Actions'
...
class ProjectRequirementsTypesList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
		...
		this.ColumnsList = ['id', 'title', 'description']
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let ProjectRequirementsTypes = {}
		if (AllFilters.ProjectRequirementsTypes !== undefined){
			ProjectRequirementsTypes = AllFilters.ProjectRequirementsTypes
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (ProjectRequirementsTypes[ColumnName] !== undefined){
				Param = ProjectRequirementsTypes[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.ProjectRequirementsTypes = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = ProjectRequirementsTypesModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchProjectRequirementsTypes(Post);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = ProjectRequirementsTypesModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/ProjectRequirementsTypes/NewItem`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/ProjectRequirementsTypes/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/ProjectRequirementsTypes/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
...
const mapStateToProps = (state, props) => {
	return {
		...
		Payloads: state.ProjectRequirementsTypesReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchProjectRequirementsTypes, deleteProjectRequirementsTypes})(ProjectRequirementsTypesList);