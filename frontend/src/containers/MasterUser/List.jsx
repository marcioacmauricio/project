...
import { fetchMasterUser, deleteMasterUser } from '../../reducers/MasterUser/Actions'
...
class MasterUserList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
		...
		this.ColumnsList = ['id', 'id_authentication']
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let MasterUser = {}
		if (AllFilters.MasterUser !== undefined){
			MasterUser = AllFilters.MasterUser
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (MasterUser[ColumnName] !== undefined){
				Param = MasterUser[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = MasterUserModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchMasterUser(Post);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = MasterUserModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/MasterUser/NewItem`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/MasterUser/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/MasterUser/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
...
const mapStateToProps = (state, props) => {
	return {
		...
		Payloads: state.MasterUserReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchMasterUser, deleteMasterUser})(MasterUserList);