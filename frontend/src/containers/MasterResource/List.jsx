...
import { fetchMasterResource, deleteMasterResource } from '../../reducers/MasterResource/Actions'
...
class MasterResourceList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let MasterResource = {}
		if (AllFilters.MasterResource !== undefined){
			MasterResource = AllFilters.MasterResource
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (MasterResource[ColumnName] !== undefined){
				Param = MasterResource[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.MasterResource = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = MasterResourceModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchMasterResource(Post);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = MasterResourceModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/MasterResource/NewItem`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/MasterResource/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/MasterResource/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
...
const mapStateToProps = (state, props) => {
	return {
		...
		Payloads: state.MasterResourceReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchMasterResource, deleteMasterResource})(MasterResourceList);