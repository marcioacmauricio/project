...
import { readMasterResource } from 'reducers/MasterResource/Actions'
...
class MasterResourceView extends React.Component {
	constructor() {
		super();
					...
					<div className="col">
						<Card className="shadow">
							<CardHeader className="border-0">
								<MenuHeader getItem={this.getItem.bind(this)} Entity="MasterResource" ItemValue={ this.state.Item.id } ItemLabel={ this.state.Item.title } View="View" />	    				
							</CardHeader>
							<CardBody>
								<CardTitle><h2>Resource</h2></CardTitle>
								<CardText tag="div" >{this.renderColumns()}</CardText>
							</CardBody>
							<CardFooter>
								<Button outline color="success" className="float-right" ><Link to='/Admin/MasterResource/ListItems'>Lista</Link></Button>							    
							</CardFooter>
						</Card>
					</div>
				</Container>
...
export default connect(mapStateToProps, {readMasterResource})(MasterResourceView);