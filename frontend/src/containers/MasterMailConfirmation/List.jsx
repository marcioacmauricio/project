...
import { fetchMasterMailConfirmation, deleteMasterMailConfirmation } from '../../reducers/MasterMailConfirmation/Actions'
...
class MasterMailConfirmationList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let MasterMailConfirmation = {}
		if (AllFilters.MasterMailConfirmation !== undefined){
			MasterMailConfirmation = AllFilters.MasterMailConfirmation
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (MasterMailConfirmation[ColumnName] !== undefined){
				Param = MasterMailConfirmation[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.MasterMailConfirmation = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = MasterMailConfirmationModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchMasterMailConfirmation(Post, this.props.match.params.IdPredesc);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = MasterMailConfirmationModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/MasterMailConfirmation/NewItem/MasterUser/${this.props.match.params.IdPredesc}`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/MasterMailConfirmation/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/MasterMailConfirmation/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
...
const mapStateToProps = (state, props) => {
	return {
		...
		Payloads: state.MasterMailConfirmationReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchMasterMailConfirmation, deleteMasterMailConfirmation})(MasterMailConfirmationList);