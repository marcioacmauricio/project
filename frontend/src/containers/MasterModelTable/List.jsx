...
import { fetchMasterModelTable, deleteMasterModelTable } from '../../reducers/MasterModelTable/Actions'
...
class MasterModelTableList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let MasterModelTable = {}
		if (AllFilters.MasterModelTable !== undefined){
			MasterModelTable = AllFilters.MasterModelTable
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (MasterModelTable[ColumnName] !== undefined){
				Param = MasterModelTable[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.MasterModelTable = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = MasterModelTableModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchMasterModelTable(Post);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = MasterModelTableModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/MasterModelTable/NewItem`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/MasterModelTable/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/MasterModelTable/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
					...
					<div className="col">
						<Card className="shadow">
							<CardHeader className="border-0">
								<MenuHeader Entity="MasterModelTable" ItemValue={ItemValue} ItemLabel={ItemLabel} View="List" />
							</CardHeader>		
							<CardBody>
								<Row>
									<Col xs="3">
...
const mapStateToProps = (state, props) => {
	return {
		...
		Payloads: state.MasterModelTableReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchMasterModelTable, deleteMasterModelTable})(MasterModelTableList);