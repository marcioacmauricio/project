...
import { createMasterModelTable, readMasterModelTable, updateMasterModelTable } from 'reducers/MasterModelTable/Actions'
...
class MasterModelTableEdit extends React.Component {
	constructor() {
		super()
	...
	onSubmit(e) {
		...
		if (this.props.match.params.id > 0){
			this.props.updateMasterModelTable(Values);
		} else {
			this.props.createMasterModelTable(Values);
		}
		e.preventDefault();	   
	}
	componentWillMount() {
	...
	componentWillReceiveProps(nextProps) {
		if (( this.props.match.params.EntityPredesc !== "" ) && ( this.props.match.params.EntityPredesc !== undefined ) && ( this.props.match.params.EntityPredesc !== null ) && (nextProps.Payload.Item !== null)){
			if (nextProps.Payload.Item.id > 0){
				this.props.history.push(`/Admin/MasterModelTable/EditItem/${nextProps.Payload.Item.id}`)
				return
			}			
		}
						...
						<Card className="shadow">
							<CardHeader className="border-0">
								...
								<ButtonGroup>
								    ...
								    <Link className="btn btn-outline-info" to="/Admin/MasterModelTable/ListItems"><i className="fa fa-times" aria-hidden="true"></i> Fechar</Link>
								</ButtonGroup>
							</CardHeader>
							<CardBody>
...
export default connect(mapStateToProps, {readMasterModelTable, createMasterModelTable, updateMasterModelTable})(MasterModelTableEdit);