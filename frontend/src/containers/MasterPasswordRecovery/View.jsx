...
import { readMasterPasswordRecovery } from 'reducers/MasterPasswordRecovery/Actions'
					...
					<div className="col">
						<Card className="shadow">
							<CardHeader className="border-0">
								<MenuHeader getItem={this.getItem.bind(this)} Entity="MasterPasswordRecovery" ItemValue={ this.state.Item.id } ItemLabel={ this.state.Item.id } View="View" />	    				
							</CardHeader>
							<CardBody>
								<CardTitle><h2>Password recovery</h2></CardTitle>
								<CardText tag="div" >{this.renderColumns()}</CardText>
							</CardBody>
							<CardFooter>
								<Button outline color="success" className="float-right" ><Link to={`/Admin/MasterPasswordRecovery/ListItems/MasterUser/${ItemValue}`}>Lista</Link></Button>							    
							</CardFooter>
						</Card>
					</div>
				</Container>
...
export default connect(mapStateToProps, {readMasterPasswordRecovery})(MasterPasswordRecoveryView);