...
import { fetchMasterPasswordRecovery, deleteMasterPasswordRecovery } from '../../reducers/MasterPasswordRecovery/Actions'
import { connect } from 'react-redux'
import MasterPasswordRecoveryModel from 'models/MasterPasswordRecoveryModel'
...
class MasterPasswordRecoveryList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
		...
		this.ColumnsList = ['id', 'id_user']
		if(this.ColumnsList.indexOf('id_user') === -1){
			this.ColumnsList.push('id_user')
		}
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let MasterPasswordRecovery = {}
		if (AllFilters.MasterPasswordRecovery !== undefined){
			MasterPasswordRecovery = AllFilters.MasterPasswordRecovery
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (MasterPasswordRecovery[ColumnName] !== undefined){
				Param = MasterPasswordRecovery[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.MasterPasswordRecovery = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = MasterPasswordRecoveryModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchMasterPasswordRecovery(Post, this.props.match.params.IdPredesc);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = MasterPasswordRecoveryModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/MasterPasswordRecovery/NewItem/MasterUser/${this.props.match.params.IdPredesc}`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/MasterPasswordRecovery/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/MasterPasswordRecovery/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
					...
					<div className="col">
						<Card className="shadow">
							<CardHeader className="border-0">
								<MenuHeader Entity="MasterPasswordRecovery" ItemValue={ItemValue} ItemLabel={ItemLabel} View="List" />
							</CardHeader>		
							<CardBody>
								<Row>
									<Col xs="3">
...
const mapStateToProps = (state, props) => {
	return {
		Payload: state.MasterPasswordRecoveryReducer.item,
		Payloads: state.MasterPasswordRecoveryReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchMasterPasswordRecovery, deleteMasterPasswordRecovery})(MasterPasswordRecoveryList);