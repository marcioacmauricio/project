...
import { createMasterPasswordRecovery, readMasterPasswordRecovery, updateMasterPasswordRecovery } from 'reducers/MasterPasswordRecovery/Actions'
...
class MasterPasswordRecoveryEdit extends React.Component {
	constructor() {
		super()
		...
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			ColumnData = MasterPasswordRecoveryModel.columns[ColumnName]
	...
	onSubmit(e) {
		...
		if (this.props.match.params.id > 0){
			this.props.updateMasterPasswordRecovery(Values);
		} else {
			this.props.createMasterPasswordRecovery(Values);
		}
		e.preventDefault();	   
	}
	componentWillMount() {
		if (this.props.match.params.id > 0){
			...
			this.props.readMasterPasswordRecovery( Posts );
		} else {
	...
	componentWillReceiveProps(nextProps) {
		if (( this.props.match.params.EntityPredesc !== "" ) && ( this.props.match.params.EntityPredesc !== undefined ) && ( this.props.match.params.EntityPredesc !== null ) && (nextProps.Payload.Item !== null)){
			if (nextProps.Payload.Item.id > 0){
				this.props.history.push(`/Admin/MasterPasswordRecovery/EditItem/${nextProps.Payload.Item.id}`)
				return
			}			
		}
	...
	renderField(ColumnName){
		let ColumnData = MasterPasswordRecoveryModel.columns[ColumnName]
		return (
						...
						<Card className="shadow">
							<CardHeader className="border-0">
								...
								<ButtonGroup>
							        ...
							        <Link className="btn btn-outline-info" to={`/Admin/MasterPasswordRecovery/ListItems/MasterUser/${ItemValue}`}><i className="fa fa-times" aria-hidden="true"></i> Fechar</Link>
								</ButtonGroup>
							</CardHeader>
							<CardBody>
...
export default connect(mapStateToProps, {readMasterPasswordRecovery, createMasterPasswordRecovery, updateMasterPasswordRecovery})(MasterPasswordRecoveryEdit);