...
import { fetchProjectRequirementTest, deleteProjectRequirementTest } from '../../reducers/ProjectRequirementTest/Actions'
...
class ProjectRequirementTestList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
		...
		this.ColumnsList = ['id', 'title', 'description', 'id_requirement']
		if(this.ColumnsList.indexOf('id_requirement') === -1){
			this.ColumnsList.push('id_requirement')
		}
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let ProjectRequirementTest = {}
		if (AllFilters.ProjectRequirementTest !== undefined){
			ProjectRequirementTest = AllFilters.ProjectRequirementTest
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (ProjectRequirementTest[ColumnName] !== undefined){
				Param = ProjectRequirementTest[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.ProjectRequirementTest = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = ProjectRequirementTestModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchProjectRequirementTest(Post, this.props.match.params.IdPredesc);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = ProjectRequirementTestModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/ProjectRequirementTest/NewItem/ProjectRequirement/${this.props.match.params.IdPredesc}`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/ProjectRequirementTest/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/ProjectRequirementTest/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
					...
					<div className="col">
						<Card className="shadow">
							<CardHeader className="border-0">
								<MenuHeader Entity="ProjectRequirementTest" ItemValue={ItemValue} ItemLabel={ItemLabel} View="List" />
							</CardHeader>		
							<CardBody>
								<Row>
									<Col xs="3">
...
const mapStateToProps = (state, props) => {
	return {
		...
		Payloads: state.ProjectRequirementTestReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchProjectRequirementTest, deleteProjectRequirementTest})(ProjectRequirementTestList);