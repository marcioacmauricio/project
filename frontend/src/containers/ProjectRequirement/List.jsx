...
import { fetchProjectRequirement, deleteProjectRequirement } from '../../reducers/ProjectRequirement/Actions'
...
class ProjectRequirementList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
		...
		this.ColumnsList = ['id', 'title', 'description', 'id_project', 'id_project_stage']
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let ProjectRequirement = {}
		if (AllFilters.ProjectRequirement !== undefined){
			ProjectRequirement = AllFilters.ProjectRequirement
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (ProjectRequirement[ColumnName] !== undefined){
				Param = ProjectRequirement[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.ProjectRequirement = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = ProjectRequirementModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchProjectRequirement(Post, this.props.match.params.IdPredesc);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = ProjectRequirementModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/ProjectRequirement/NewItem/ProjectProject/${this.props.match.params.IdPredesc}`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/ProjectRequirement/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/ProjectRequirement/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
...
const mapStateToProps = (state, props) => {
	return {
		...
		Payloads: state.ProjectRequirementReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchProjectRequirement, deleteProjectRequirement})(ProjectRequirementList);