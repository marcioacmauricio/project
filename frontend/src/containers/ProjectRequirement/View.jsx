...
import { readProjectRequirement } from 'reducers/ProjectRequirement/Actions'
					...
					<div className="col">
						<Card className="shadow">
							<CardHeader className="border-0">
								<MenuHeader getItem={this.getItem.bind(this)} Entity="ProjectRequirement" ItemValue={ this.state.Item.id } ItemLabel={ this.state.Item.title } View="View" />	    				
							</CardHeader>
							<CardBody>
								<CardTitle><h2>Requirements</h2></CardTitle>
								<CardText tag="div" >{this.renderColumns()}</CardText>
							</CardBody>
							<CardFooter>
								<Button outline color="success" className="float-right" ><Link to={`/Admin/ProjectRequirement/ListItems/ProjectProject/${ItemValue}`}>Lista</Link></Button>							    
							</CardFooter>
						</Card>
					</div>
				</Container>
...
export default connect(mapStateToProps, {readProjectRequirement})(ProjectRequirementView);