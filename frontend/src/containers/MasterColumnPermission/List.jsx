...
import { fetchMasterColumnPermission, deleteMasterColumnPermission } from '../../reducers/MasterColumnPermission/Actions'
...
class MasterColumnPermissionList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
		...
		this.ColumnsList = ['id', 'id_model_table', 'id_table_permission', 'permission']
		if(this.ColumnsList.indexOf('id_model_table') === -1){
			this.ColumnsList.push('id_model_table')
		}
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let MasterColumnPermission = {}
		if (AllFilters.MasterColumnPermission !== undefined){
			MasterColumnPermission = AllFilters.MasterColumnPermission
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (MasterColumnPermission[ColumnName] !== undefined){
				Param = MasterColumnPermission[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.MasterColumnPermission = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = MasterColumnPermissionModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchMasterColumnPermission(Post, this.props.match.params.IdPredesc);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = MasterColumnPermissionModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/MasterColumnPermission/NewItem/MasterModelTable/${this.props.match.params.IdPredesc}`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/MasterColumnPermission/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/MasterColumnPermission/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
					...
					<div className="col">
						<Card className="shadow">
							<CardHeader className="border-0">
								<MenuHeader Entity="MasterColumnPermission" ItemValue={ItemValue} ItemLabel={ItemLabel} View="List" />
							</CardHeader>		
							<CardBody>
								<Row>
									<Col xs="3">
...
const mapStateToProps = (state, props) => {
	return {
		...
		Payloads: state.MasterColumnPermissionReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchMasterColumnPermission, deleteMasterColumnPermission})(MasterColumnPermissionList);