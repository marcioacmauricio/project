...
import { createProjectProjectStage, readProjectProjectStage, updateProjectProjectStage } from 'reducers/ProjectProjectStage/Actions'
...
class ProjectProjectStageEdit extends React.Component {
	constructor() {
		super()
	...
	onSubmit(e) {
		...
		if (this.props.match.params.id > 0){
			this.props.updateProjectProjectStage(Values);
		} else {
			this.props.createProjectProjectStage(Values);
		}
		e.preventDefault();	   
	}
	componentWillMount() {
	...
	componentWillReceiveProps(nextProps) {
		if (( this.props.match.params.EntityPredesc !== "" ) && ( this.props.match.params.EntityPredesc !== undefined ) && ( this.props.match.params.EntityPredesc !== null ) && (nextProps.Payload.Item !== null)){
			if (nextProps.Payload.Item.id > 0){
				this.props.history.push(`/Admin/ProjectProjectStage/EditItem/${nextProps.Payload.Item.id}`)
				return
			}			
		}
						...
						<Card className="shadow">
							<CardHeader className="border-0">
								...
								<ButtonGroup>
							        ...
							        <Link className="btn btn-outline-info" to={`/Admin/ProjectProjectStage/ListItems/ProjectProject/${ItemValue}`}><i className="fa fa-times" aria-hidden="true"></i> Fechar</Link>
								</ButtonGroup>
							</CardHeader>
							<CardBody>
...
export default connect(mapStateToProps, {readProjectProjectStage, createProjectProjectStage, updateProjectProjectStage})(ProjectProjectStageEdit);