...
import { fetchProjectProjectStage, deleteProjectProjectStage } from '../../reducers/ProjectProjectStage/Actions'
...
class ProjectProjectStageList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
		...
		this.ColumnsList = ['id', 'title', 'description', 'id_project']
		if(this.ColumnsList.indexOf('id_project') === -1){
			this.ColumnsList.push('id_project')
		}
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let ProjectProjectStage = {}
		if (AllFilters.ProjectProjectStage !== undefined){
			ProjectProjectStage = AllFilters.ProjectProjectStage
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (ProjectProjectStage[ColumnName] !== undefined){
				Param = ProjectProjectStage[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.ProjectProjectStage = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = ProjectProjectStageModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchProjectProjectStage(Post, this.props.match.params.IdPredesc);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = ProjectProjectStageModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/ProjectProjectStage/NewItem/ProjectProject/${this.props.match.params.IdPredesc}`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/ProjectProjectStage/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/ProjectProjectStage/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
					...
					<div className="col">
						<Card className="shadow">
							<CardHeader className="border-0">
								<MenuHeader Entity="ProjectProjectStage" ItemValue={ItemValue} ItemLabel={ItemLabel} View="List" />
							</CardHeader>		
							<CardBody>
								<Row>
									<Col xs="3">
...
const mapStateToProps = (state, props) => {
	return {
		...
		Payloads: state.ProjectProjectStageReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchProjectProjectStage, deleteProjectProjectStage})(ProjectProjectStageList);