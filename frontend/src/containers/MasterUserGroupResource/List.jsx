...
import { fetchMasterUserGroupResource, deleteMasterUserGroupResource } from '../../reducers/MasterUserGroupResource/Actions'
...
class MasterUserGroupResourceList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
		...
		this.ColumnsList = ['id', 'id_user_group', 'id_resource']
		if(this.ColumnsList.indexOf('id_user_group') === -1){
			this.ColumnsList.push('id_user_group')
		}
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let MasterUserGroupResource = {}
		if (AllFilters.MasterUserGroupResource !== undefined){
			MasterUserGroupResource = AllFilters.MasterUserGroupResource
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (MasterUserGroupResource[ColumnName] !== undefined){
				Param = MasterUserGroupResource[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.MasterUserGroupResource = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = MasterUserGroupResourceModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchMasterUserGroupResource(Post, this.props.match.params.IdPredesc);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = MasterUserGroupResourceModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/MasterUserGroupResource/NewItem/MasterUserGroup/${this.props.match.params.IdPredesc}`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/MasterUserGroupResource/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/MasterUserGroupResource/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
...
const mapStateToProps = (state, props) => {
	return {
		...
		Payloads: state.MasterUserGroupResourceReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchMasterUserGroupResource, deleteMasterUserGroupResource})(MasterUserGroupResourceList);