...
import { fetchProjectProject, deleteProjectProject } from '../../reducers/ProjectProject/Actions'
...
class ProjectProjectList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let ProjectProject = {}
		if (AllFilters.ProjectProject !== undefined){
			ProjectProject = AllFilters.ProjectProject
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (ProjectProject[ColumnName] !== undefined){
				Param = ProjectProject[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.ProjectProject = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = ProjectProjectModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchProjectProject(Post);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = ProjectProjectModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/ProjectProject/NewItem`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/ProjectProject/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/ProjectProject/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
...
const mapStateToProps = (state, props) => {
	return {
		...
		Payloads: state.ProjectProjectReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchProjectProject, deleteProjectProject})(ProjectProjectList);