...
import { createMasterUserGroup, readMasterUserGroup, updateMasterUserGroup } from 'reducers/MasterUserGroup/Actions'
import MasterUserGroupModel from 'models/MasterUserGroupModel'
import Fields from 'fields/Fields'
import { MenuHeader, HeaderAdmin } from 'components/Headers'
class MasterUserGroupEdit extends React.Component {
	constructor() {
		super()
	...
	onSubmit(e) {
		...
		if (this.props.match.params.id > 0){
			this.props.updateMasterUserGroup(Values);
		} else {
			this.props.createMasterUserGroup(Values);
		}
		e.preventDefault();	   
	}
	componentWillMount() {
	...
	componentWillReceiveProps(nextProps) {
		if (( this.props.match.params.EntityPredesc !== "" ) && ( this.props.match.params.EntityPredesc !== undefined ) && ( this.props.match.params.EntityPredesc !== null ) && (nextProps.Payload.Item !== null)){
			if (nextProps.Payload.Item.id > 0){
				this.props.history.push(`/Admin/MasterUserGroup/EditItem/${nextProps.Payload.Item.id}`)
				return
			}			
		}
						...
						<Card className="shadow">
							<CardHeader className="border-0">
								...
								<ButtonGroup>
								    ...
								    <Link className="btn btn-outline-info" to="/Admin/MasterUserGroup/ListItems"><i className="fa fa-times" aria-hidden="true"></i> Fechar</Link>
								</ButtonGroup>
							</CardHeader>
							<CardBody>
...
export default connect(mapStateToProps, {readMasterUserGroup, createMasterUserGroup, updateMasterUserGroup})(MasterUserGroupEdit);