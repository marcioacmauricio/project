...
import { createMasterTablePermission, readMasterTablePermission, updateMasterTablePermission } from 'reducers/MasterTablePermission/Actions'
...
class MasterTablePermissionEdit extends React.Component {
	constructor() {
		...
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			ColumnData = MasterTablePermissionModel.columns[ColumnName]
	...
	onSubmit(e) {
		...
		if (this.props.match.params.id > 0){
			this.props.updateMasterTablePermission(Values);
		} else {
			this.props.createMasterTablePermission(Values);
		}
		e.preventDefault();	   
	}
	componentWillMount() {
		if (this.props.match.params.id > 0){
			...
			this.props.readMasterTablePermission( Posts );
		} else {
	...
	componentWillReceiveProps(nextProps) {
		if (( this.props.match.params.EntityPredesc !== "" ) && ( this.props.match.params.EntityPredesc !== undefined ) && ( this.props.match.params.EntityPredesc !== null ) && (nextProps.Payload.Item !== null)){
			if (nextProps.Payload.Item.id > 0){
				this.props.history.push(`/Admin/MasterTablePermission/EditItem/${nextProps.Payload.Item.id}`)
				return
			}			
		}
	...
	renderField(ColumnName){
		let ColumnData = MasterTablePermissionModel.columns[ColumnName]
		return (
						...
						<Card className="shadow">
							<CardHeader className="border-0">
								...
								<ButtonGroup>
								    ...
								    <Link className="btn btn-outline-info" to="/Admin/MasterTablePermission/ListItems"><i className="fa fa-times" aria-hidden="true"></i> Fechar</Link>
								</ButtonGroup>
							</CardHeader>
							<CardBody>
								...
								<CardText tag="div">
									{this.renderAlert()}
									<Form>
										{this.renderField('table_name')}
										{this.renderField('permission')}
									</Form>
								</CardText>
							</CardBody>
							<CardFooter>
...
export default connect(mapStateToProps, {readMasterTablePermission, createMasterTablePermission, updateMasterTablePermission})(MasterTablePermissionEdit);