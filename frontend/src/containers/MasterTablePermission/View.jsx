...
import { readMasterTablePermission } from 'reducers/MasterTablePermission/Actions'
...
class MasterTablePermissionView extends React.Component {
	constructor() {
		super();
					...
					<div className="col">
						<Card className="shadow">
							<CardHeader className="border-0">
								<MenuHeader getItem={this.getItem.bind(this)} Entity="MasterTablePermission" ItemValue={ this.state.Item.id } ItemLabel={ this.state.Item.id } View="View" />	    				
							</CardHeader>
							<CardBody>
								<CardTitle><h2>Table permission</h2></CardTitle>
								<CardText tag="div" >{this.renderColumns()}</CardText>
							</CardBody>
							<CardFooter>
								<Button outline color="success" className="float-right" ><Link to='/Admin/MasterTablePermission/ListItems'>Lista</Link></Button>							    
							</CardFooter>
						</Card>
					</div>
				</Container>
...
export default connect(mapStateToProps, {readMasterTablePermission})(MasterTablePermissionView);