...
import { fetchMasterTablePermission, deleteMasterTablePermission } from '../../reducers/MasterTablePermission/Actions'
import { connect } from 'react-redux'
import MasterTablePermissionModel from 'models/MasterTablePermissionModel'
...
class MasterTablePermissionList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
		...
		this.ColumnsList = ['id', 'permission']
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let MasterTablePermission = {}
		if (AllFilters.MasterTablePermission !== undefined){
			MasterTablePermission = AllFilters.MasterTablePermission
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (MasterTablePermission[ColumnName] !== undefined){
				Param = MasterTablePermission[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.MasterTablePermission = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = MasterTablePermissionModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchMasterTablePermission(Post);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = MasterTablePermissionModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/MasterTablePermission/NewItem`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/MasterTablePermission/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/MasterTablePermission/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
					...
					<div className="col">
						<Card className="shadow">
							<CardHeader className="border-0">
								<MenuHeader Entity="MasterTablePermission" ItemValue={ItemValue} ItemLabel={ItemLabel} View="List" />
							</CardHeader>		
							<CardBody>
								<Row>
									<Col xs="3">
...
const mapStateToProps = (state, props) => {
	return {
		Payload: state.MasterTablePermissionReducer.item,
		Payloads: state.MasterTablePermissionReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchMasterTablePermission, deleteMasterTablePermission})(MasterTablePermissionList);