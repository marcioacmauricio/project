...
import { fetchMasterAuthentication, deleteMasterAuthentication } from '../../reducers/MasterAuthentication/Actions'
...
class MasterAuthenticationList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
		...
		this.ColumnsList = ['id', 'id_user', 'status_logged']
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let MasterAuthentication = {}
		if (AllFilters.MasterAuthentication !== undefined){
			MasterAuthentication = AllFilters.MasterAuthentication
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (MasterAuthentication[ColumnName] !== undefined){
				Param = MasterAuthentication[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.MasterAuthentication = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = MasterAuthenticationModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchMasterAuthentication(Post, this.props.match.params.IdPredesc);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = MasterAuthenticationModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/MasterAuthentication/NewItem/MasterUser/${this.props.match.params.IdPredesc}`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/MasterAuthentication/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/MasterAuthentication/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
...
const mapStateToProps = (state, props) => {
	return {
		...
		Payloads: state.MasterAuthenticationReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchMasterAuthentication, deleteMasterAuthentication})(MasterAuthenticationList);