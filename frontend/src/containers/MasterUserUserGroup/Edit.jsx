...
import { createMasterUserUserGroup, readMasterUserUserGroup, updateMasterUserUserGroup } from 'reducers/MasterUserUserGroup/Actions'
...
class MasterUserUserGroupEdit extends React.Component {
	constructor() {
		super()
	...
	onSubmit(e) {
		...
		if (this.props.match.params.id > 0){
			this.props.updateMasterUserUserGroup(Values);
		} else {
			this.props.createMasterUserUserGroup(Values);
		}
		e.preventDefault();	   
	}
	componentWillMount() {
	...
	componentWillReceiveProps(nextProps) {
		if (( this.props.match.params.EntityPredesc !== "" ) && ( this.props.match.params.EntityPredesc !== undefined ) && ( this.props.match.params.EntityPredesc !== null ) && (nextProps.Payload.Item !== null)){
			if (nextProps.Payload.Item.id > 0){
				this.props.history.push(`/Admin/MasterUserUserGroup/EditItem/${nextProps.Payload.Item.id}`)
				return
			}			
		}
						...
						<Card className="shadow">
							<CardHeader className="border-0">
								...
								<ButtonGroup>
							        ...
							        <Link className="btn btn-outline-info" to={`/Admin/MasterUserUserGroup/ListItems/MasterUser/${ItemValue}`}><i className="fa fa-times" aria-hidden="true"></i> Fechar</Link>
								</ButtonGroup>
							</CardHeader>
							<CardBody>
								...
								<CardText tag="div">
									{this.renderAlert()}
									<Form>
										{this.renderField('id_user_group')}
									</Form>
								</CardText>
							</CardBody>
							<CardFooter>
...
export default connect(mapStateToProps, {readMasterUserUserGroup, createMasterUserUserGroup, updateMasterUserUserGroup})(MasterUserUserGroupEdit);