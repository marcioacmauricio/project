...
import { fetchMasterUserUserGroup, deleteMasterUserUserGroup } from '../../reducers/MasterUserUserGroup/Actions'
...
class MasterUserUserGroupList extends React.Component {
	constructor() {
		super();
		this.state = {
		    Search: "",
		...
		this.ColumnsList = ['id', 'id_user', 'id_user_group']
	...
	getDataFilter(ColName = undefined, Value = undefined){
		...
		let MasterUserUserGroup = {}
		if (AllFilters.MasterUserUserGroup !== undefined){
			MasterUserUserGroup = AllFilters.MasterUserUserGroup
		} 
		let Filters = {}
		for (let i = 0; i < this.ColumnsList.length; i++){
			...
			if (MasterUserUserGroup[ColumnName] !== undefined){
				Param = MasterUserUserGroup[ColumnName]
			} else {
				if (ColumnName === 'id') {
					Param.order = "ASC"
				}
		...
		AllFilters.MasterUserUserGroup = Filters		
	...
	componentWillMount() {
		...
		for (let ColumnName in DataFilter){
			...
			let ColumnMetaData = MasterUserUserGroupModel.columns[ColumnName]
	...
	fetchItems(DataFilter){
		...
		this.props.fetchMasterUserUserGroup(Post, this.props.match.params.IdPredesc);
	}
	...
	renderTR(){
		...
		for (let ColumnName in DataFilter){
			...
			ColumnMetaData = MasterUserUserGroupModel.columns[ColumnName]
			let ClassIcon = ''
			if (DataFilter[ColumnName].order === null){
		...
		return {
			Trs: (
				<thead className="thead-light">
					<tr>
						<th><ButtonGroup><Link className="btn btn-link" to={`/Admin/MasterUserUserGroup/NewItem/MasterUser/${this.props.match.params.IdPredesc}`}><i className="fa fa-plus" aria-hidden="true"></i></Link><Button onClick={this.ShowFilters.bind(this)} color="link"><i className="fa fa-search-plus" aria-hidden="true"></i></Button></ButtonGroup></th>					
						{Trs}
					</tr>
				</thead>
			),
	...
	renderRow(Item){
		...
		let ColumnData = {}
		let DataFilter = this.getDataFilter()		
		Cels.push(<td key="button"><ButtonGroup><Link className="btn btn-outline-info" to={`/Admin/MasterUserUserGroup/ShowItem/${Item.id}`} ><i className="fa fa-check" aria-hidden="true"></i></Link><Link className="btn btn-outline-info" to={`/Admin/MasterUserUserGroup/EditItem/${Item.id}`}><i className="fa fa-edit" aria-hidden="true"></i></Link><Button outline color="danger"><i className="fa fa-trash" aria-hidden="true"></i></Button></ButtonGroup></td>)
					...
					<div className="col">
						<Card className="shadow">
							<CardHeader className="border-0">
								<MenuHeader Entity="MasterUserUserGroup" ItemValue={ItemValue} ItemLabel={ItemLabel} View="List" />
							</CardHeader>		
							<CardBody>
								<Row>
									<Col xs="3">
...
const mapStateToProps = (state, props) => {
	return {
		...
		Payloads: state.MasterUserUserGroupReducer.items,
		auth: state.Register.auth
	}
}
export default connect(mapStateToProps, {fetchMasterUserUserGroup, deleteMasterUserUserGroup})(MasterUserUserGroupList);