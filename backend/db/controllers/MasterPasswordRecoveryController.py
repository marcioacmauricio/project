...
class MasterPasswordRecoveryController(SuperController):
	def __init__(self, DB, User, Key = None, ToSave = [], ToList = [], ToView = [], Item = []):
		self.DB = DB
		self.User = User
		super(MasterPasswordRecoveryController, self).__init__(MasterPasswordRecoveryModel, Key, ToSave, ToList, ToView, Item)
