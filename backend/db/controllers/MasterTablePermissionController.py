...
class MasterTablePermissionController(SuperController):
	def __init__(self, DB, User, Key = None, ToSave = [], ToList = [], ToView = [], Item = []):
		self.DB = DB
		self.User = User
		super(MasterTablePermissionController, self).__init__(MasterTablePermissionModel, Key, ToSave, ToList, ToView, Item)
