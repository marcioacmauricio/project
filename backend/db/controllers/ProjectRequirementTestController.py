...
class ProjectRequirementTestController(SuperController):
	def __init__(self, DB, User, Key = None, ToSave = [], ToList = [], ToView = [], Item = []):
		self.DB = DB
		self.User = User
		super(ProjectRequirementTestController, self).__init__(ProjectRequirementTestModel, Key, ToSave, ToList, ToView, Item)
