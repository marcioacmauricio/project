...
class MasterMailConfirmationController(SuperController):
	def __init__(self, DB, User, Key = None, ToSave = [], ToList = [], ToView = [], Item = []):
		self.DB = DB
		self.User = User
		super(MasterMailConfirmationController, self).__init__(MasterMailConfirmationModel, Key, ToSave, ToList, ToView, Item)
