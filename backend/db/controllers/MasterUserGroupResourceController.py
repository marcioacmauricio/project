...
class MasterUserGroupResourceController(SuperController):
	def __init__(self, DB, User, Key = None, ToSave = [], ToList = [], ToView = [], Item = []):
		self.DB = DB
		self.User = User
		super(MasterUserGroupResourceController, self).__init__(MasterUserGroupResourceModel, Key, ToSave, ToList, ToView, Item)
