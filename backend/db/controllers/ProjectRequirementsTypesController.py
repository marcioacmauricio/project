...
class ProjectRequirementsTypesController(SuperController):
	def __init__(self, DB, User, Key = None, ToSave = [], ToList = [], ToView = [], Item = []):
		self.DB = DB
		self.User = User
		super(ProjectRequirementsTypesController, self).__init__(ProjectRequirementsTypesModel, Key, ToSave, ToList, ToView, Item)
