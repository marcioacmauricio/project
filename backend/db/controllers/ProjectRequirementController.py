...
class ProjectRequirementController(SuperController):
	def __init__(self, DB, User, Key = None, ToSave = [], ToList = [], ToView = [], Item = []):
		self.DB = DB
		self.User = User
		super(ProjectRequirementController, self).__init__(ProjectRequirementModel, Key, ToSave, ToList, ToView, Item)
