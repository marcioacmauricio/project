...
class MasterColumnPermissionController(SuperController):
	def __init__(self, DB, User, Key = None, ToSave = [], ToList = [], ToView = [], Item = []):
		self.DB = DB
		self.User = User
		super(MasterColumnPermissionController, self).__init__(MasterColumnPermissionModel, Key, ToSave, ToList, ToView, Item)
