...
class ProjectProjectStageController(SuperController):
	def __init__(self, DB, User, Key = None, ToSave = [], ToList = [], ToView = [], Item = []):
		self.DB = DB
		self.User = User
		super(ProjectProjectStageController, self).__init__(ProjectProjectStageModel, Key, ToSave, ToList, ToView, Item)
