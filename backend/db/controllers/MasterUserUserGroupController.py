...
class MasterUserUserGroupController(SuperController):
	def __init__(self, DB, User, Key = None, ToSave = [], ToList = [], ToView = [], Item = []):
		self.DB = DB
		self.User = User
		super(MasterUserUserGroupController, self).__init__(MasterUserUserGroupModel, Key, ToSave, ToList, ToView, Item)
